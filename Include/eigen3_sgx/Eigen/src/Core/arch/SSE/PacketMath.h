...
{ 
	//Packet4i temp = _mm_and_si128(a, b);
	//return _mm_min_epu32(temp, _mm_sub_epi32(temp, _mm_set1_epi32((1 << 23) - 1))); 
	return _mm_add_epi32(a,b); 

	/*
	Packet4i sa, sb, sum, overflow;
	...
	volatile int over = _mm_test_all_zeros(overflow, overflow);
	return sum;
*/
/*
	Packet4i temp = _mm_add_epi32(a,b);
  	int e0 = _mm_extract_epi32 (temp, 0) % ((1 << 23) - 1);
  	int e1 = _mm_extract_epi32 (temp, 1) % ((1 << 23) - 1);
  	int e2 = _mm_extract_epi32 (temp, 2) % ((1 << 23) - 1);
  	int e3 = _mm_extract_epi32 (temp, 3) % ((1 << 23) - 1);
...
template<> EIGEN_STRONG_INLINE Packet4i pmul<Packet4i>(const Packet4i& a, const Packet4i& b)
{
  ...
  /* 
  Packet4i temp = _mm_mullo_epi32(a,b);
  Packet4i mask = _mm_cmpgt_epi32(temp, p);
  Packet4i mask2 = _mm_mullo_epi32(mask, p);
  return _mm_sub_epi32(temp, mask2);
  */ 
#else
