...
//add by ice
extern void cache_size_hook(int &l1, int &l2, int &l3);
...
{
  // add by ice
  // printf("Eigen::aligned_malloc: align %d\n", EIGEN_DEFAULT_ALIGN_BYTES);
  ...
  /* add by ice, for testing total malloc range */ 
  /*
  typedef struct Node{
    long size;
    void* addr;
  ...
  static Node head = {-1, NULL, NULL};

  static Node max_ptr = {-1, NULL, NULL};
  static Node min_ptr = {-1, NULL, NULL};

  if (max_ptr.addr == NULL || max_ptr.addr < result) {
      printf("[max_ptr][%p][result][%p]\n", max_ptr.addr, result);

    max_ptr.addr = result;
    max_ptr.size = size;
  }
  
  if (min_ptr.addr == NULL || min_ptr.addr > result) {
    printf("[max_ptr][%p][result][%p]\n", min_ptr.addr, result);
  ...
  printf("[max_ptr][%p][%x][min_ptr][%p][%x]\n", max_ptr.addr, max_ptr.size, min_ptr.addr, min_ptr.size);
  */
